  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', minHeight: '100vh', fontFamily: 'DM Sans, sans-serif', background: '#0f0f11', color: '#f0eff5' }}>
      ...
      <main style={{ padding: '36px 40px', maxWidth: 680 }}>
        ...
        {exito && (
          <div style={{ background: 'rgba(59,191,163,0.15)', border: '1px solid #3bbfa3', borderRadius: 10, padding: '12px 16px', marginBottom: 20, color: '#3bbfa3', fontSize: '0.88rem' }}>
          ...
          {subtareas.map((s, i) => (
            <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#0f0f11', border: '1px solid #2a2a32', borderRadius: 10, padding: '10px 14px', marginBottom: 8 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '0.75rem', color: '#6b6a7a' }}>{s.fecha || 'Sin fecha'} · {s.horas}h</div>
              </div>
              <button onClick={() => setSubtareas(subtareas.filter((_, j) => j !== i))}
                style={{ background: 'none', border: 'none', color: '#f04a4a', cursor: 'pointer', fontSize: '0.8rem' }}>
                Eliminar
              </button>
            </div>
          ))}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 10, alignItems: 'start' }}>
              ...
              <Campo>
                <input type="date" style={{ ...inputStyle(), width: 140 }}
                  value={nuevaSub.fecha} onChange={e => setNuevaSub({ ...nuevaSub, fecha: e.target.value })} />
              </Campo>
              <Campo error={errores.horas}>
                <input type="number" style={{ ...inputStyle(errores.horas), width: 80 }} placeholder="Horas" min="0"
